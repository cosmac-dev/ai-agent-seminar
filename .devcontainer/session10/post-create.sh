...
# session09_hitl_sbx.ipynb のサンドボックスは --pull=never で実行するため、
...
if ! command -v docker >/dev/null 2>&1; then
  echo "[session09] docker CLI が見つかりません。devcontainer.json の docker-outside-of-docker feature が有効か確認してください。" >&2
  exit 0
fi
...
if ! docker version >/dev/null 2>&1; then
  cat >&2 <<'MSG'
[session09] ホストの Docker デーモンに接続できません。次を確認してください。
...
if docker image inspect "${SANDBOX_IMAGE}" >/dev/null 2>&1; then
  echo "[session09] サンドボックス用イメージを確認しました: ${SANDBOX_IMAGE}"
  exit 0
fi

echo "[session09] サンドボックス用イメージを取得します: ${SANDBOX_IMAGE}"
if ! docker pull "${SANDBOX_IMAGE}"; then
  echo "[session09] イメージの取得に失敗しました。ネットワークを確認し、docker pull ${SANDBOX_IMAGE} を手動で実行してください。" >&2
fi
